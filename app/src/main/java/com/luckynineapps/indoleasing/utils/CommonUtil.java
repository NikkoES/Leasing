...
//        Glide.with(context).load(Constans.WEB_URL_IMAGE + url).placeholder(circularProgressDrawable).into(imageView);
    ...
    public static String addDaysToDate(String tanggal, String format, int add) {
        SimpleDateFormat formatter = new SimpleDateFormat(format);

        Calendar c = Calendar.getInstance();
        try {
            c.setTime(formatter.parse(tanggal));
        } catch (ParseException e) {
            e.printStackTrace();
        }
...
//        Date date = new Date();
    ...
    public static boolean validateEmptyEntries(ArrayList<View> fields) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if (currentField.getText().toString().length() <= 0) {
                currentField.setError("Wajib diisi !");
                check = false;
            }
        }
        return check;
    ...
    public static void editTextCurrency(final EditText editText) {
